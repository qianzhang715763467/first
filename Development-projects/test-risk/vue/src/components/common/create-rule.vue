<template>
	<div id="create-rule" v-if="action">
		...
		<!-- 接口列表 -->
		<interface-list v-show="module_display.interfaces" ref="Interface" @upup="interfaceSelection"></interface-list>
		<!-- 计算字段配置 -->
		<calculate-field v-show="module_display.calculated_field" ref="calculateField" :param="moudule_data.calculated_field"  @upup="calculatedFieldConfig"></calculate-field>
		<!-- 计算依赖字段配置 -->
...
<script>
	...
	import CalculateField			from "./rule-config-modules/calculate_field.vue" // 接口计算字段列表
		...
		watch:{
			response(curVal,oldval){ 
				if(curVal.code == 1){
					...
					}else if(curVal.name == 'interfaceData'){
						this.$refs.Interface.pageData.values = curVal.message;
					}else{// 当前规则数据请求成功,格式化数据并显示
		...
		methods:{
			ready(){// 初始化
				...
				if(this.$route.query.action == 'modify'){// 修改规则
					this.sendRequest(this,'/rule/ruleGoodsCatagory?select_rows={"rule_id":'+this.$route.query.id+'}','response');// 当要修改的规则所有数据
				}
				this.action = true;
			},
			...
			interfaceSelection(msg){// 点击“计算字段”按钮，显示接口模块
				if(msg == 'request'){// 发出请求
					this.sendRequest(this,'/interface/interfaceGoodsCatagory?select_rows={}','response','interfaceData');// 所有接口数据
					...
					if(msg.id > -1){
						...
						this.calculatedFieldDataFormat(JSON.parse(msg.val));
						this.$refs.calculateField.pageData.values = this.moudule_data.calculated_field;
					}
				}
			},
			...
			subData(){
				var $keyArr = ['rule_name','interface_id','desc','enabled','approve_action','cal_depend_field_type','cal_type_2_cal_field','cal_type_3_cal_field','cal_type_3_cal_dep_field','submit_field'],
				...
				if(this.pageData.defaultMatchingMode == '加权计算'){
					...
					if(modifyData.calculate_field.length < 1){
						...
						// 默认为空
						submitData.calculate_type	= null;
						submitData.threshold		= null;
						submitData.base_weight	 	= null;
						submitData.cfit_weight 		= null;
					}
				}
				// 计算字段存在数据，格式化
				if(modifyData.calculate_field.length > 1){
