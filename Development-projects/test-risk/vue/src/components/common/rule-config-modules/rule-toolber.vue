<template>
	<div id="rule-toolber">
		<ul class="rule-toolber">
			<!-- 详情和添加页跳转过来 -->
			<li class="rule-toolber-item" v-if="params.name">
				<div class="rule-tool rule-tool-add prohibit" v-if="params.values.added">添加</div><!--不可添加状态-->
				<template v-else>
					<!--<div>添加</div>-->
			...
			<li class="rule-toolber-item" v-else>
				...
				<div class="rule-tool rule-tool-change"@click="rule_action('修改')" :class="[params.values.enabled? 'prohibit' :'']">修改</div>
				<div class="rule-tool rule-tool-remove"@click="rule_action('删除')" :class="[params.values.enabled? 'prohibit' :'']">删除</div>
			</li>
