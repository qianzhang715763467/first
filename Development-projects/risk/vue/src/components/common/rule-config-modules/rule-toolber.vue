	...
	<div id="rule-toolber">
		<ul class="rule-toolber">
			...
			<li class="rule-toolber-item" v-else>
				...
				<div class="rule-tool rule-tool-change"@click="rule_action('修改')" :class="[params.values.status != null? 'prohibit' :'']">修改</div>
				<div class="rule-tool rule-tool-remove"@click="rule_action('删除')" :class="[params.values.status != null? 'prohibit' :'']">删除</div>
			</li>
