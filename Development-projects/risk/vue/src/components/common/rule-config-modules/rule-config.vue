	...
	export default{
		...
		watch:{
			initData(curVal,oldVal){
				if(curVal){
					var arr = format_interface_data(curVal);
					var newArr = [];
					arr.ergodic('val').unique().map(function(index){
						newArr.push(arr[index])
					})
					this.$refs.calculated.pageData.values = newArr;
					if(newArr.length < 1){
						this.$refs.DependField.select_text="";
						this.$refs.DependField.pageData.sub_field= {};
						this.$refs.DependField.pageData.htmlText = '';
						this.$refs.DependField.pageData.values = [];
					}
				}
			},
		...
		methods:{
			...
			dependField(msg){// 对应接口的 计算依赖字段
				...
				if(msg.count > 0){// 选择的"计算依赖字段" 为数组
					...
					if(msg.count == 1){// 选择的"计算依赖字段" 为数组中的非object格式值，或者数组自身
						this.constData.type_1_v = '数组长度='+arr.length;
			...
			resetField(stop){
				this.constData.type_3_v 	= "";
				this.constData.type_3_k 	= "";
				this.constData.type_2_v 	= "";
				this.constData.type_2_k 	= "";
				this.constData.type_1_v 	= "";
				this.constData.type_1_k 	= "";
				this.constData.depend_type 	= "";
				if(stop == 2)return;
				this.constData.depend_v		= "";
				this.constData.depend_k 	= "";
				if(stop == 1)return;
				this.constData.calculate_v 	= "";
				this.constData.calculate_k 	= "";
			},
