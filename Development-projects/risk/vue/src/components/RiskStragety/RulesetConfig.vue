        ...
        watch:{
        	responseStatus(currentVal,oldVal){
        		if(currentVal.code == '1'){
        			this.pageData.values = currentVal.message.JSONunique();;
        ...
        methods:{
        	...
        	changeAssociatedData(emitVal){ // "增加"|"取消增加"规则集，点击之后的返回整条规则集参数。
        		if(emitVal.action == "add"){// 将"当前规则集参数"暂存
	        		...
	        		if(this.pageData.state == 'create'){console.log(1)
                	...
                	if(this.pageData.state == 'create'){
                		this.$global.$splice(this.createData.rawData.values,emitVal.id);
                	}else{
                		this.$global.$splice(this.detailData.variableData.values,emitVal.id);
                	}
        		}
        	}
        }
