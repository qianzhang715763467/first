    ...
    <div class="details-wrap" id="detailInfo" v-if="action">
        <header class="details-head details-wrap-moudel">
            ...
            <ul class="details-head-toolbar"><!-- 工具栏 -->
            	...
            	<router-link tag="li" :to="{name:'add',query:{cRouter:this.details.detailName =='product'?'product':'ruleset',action:'copy'}}" v-show="!details.modify" @click.native="copy()"
        ...
        <section class="details-main details-wrap-moudel">
            ...
            <div class="details-main-tabBox">
                ...
                <template v-if="details.detailName == 'product'">
                    <div class="ruleSet-card"
                        ...
                        <div class="card-toolber" v-show="details.modify&&details.rawData.state != '上架中'">
                            ...
                            <!--<p class="card-tool card-tool-middle card-tool-handover"
                            ...
                            <!--<p class="card-tool card-tool-middle card-tool-handover" v-if="row.isCloud==1"
                               @click="productInclusiveRuleset_action({'action':'handover',state:'state1','val':row})">
                                <svg-icon class="icons"
                                          :class="(row.isCloud==1?[row.action_state1 > -1? cardStatusStyle[row.action_state1] : false] ? 'noDrop')"
                                          id="icon-2xuanzhong"></svg-icon>
                            </p>-->
                ...
                <!-- 显示规则模块 -->
                <template v-if="details.detailName == 'group' && (details.modify? details.variableData.values.length>0: details.rawData.values.length>0)">
                    <table border="1" cellspacing="0" cellpadding="0" >
                        ...
                        <template v-for="row in (details.modify? details.variableData.values: details.rawData.values)">
                            <tr ref="deleteTr" :class="{noDrop:(row.status == 0? true:false)}">
    ...
    export default {
        ...
        watch: {// 监听 axios 请求是否返回数据
            ...
            responseMessage(newVal,oldVal){
            	if(newVal.name){
            		if(newVal.name == 'associatedRule'){
            			this.details.rawData.values = newVal.message;
						// 复制一份原始数据，用于修改操作时使用
        ...
        methods: {
            ready() {
                ...
                if (!this.$route.query.action && this.details.modify != 'back') { // 当前动作不是在修改数据。(进入详情页面有两种动作  查看详细数据 & 修改详细数据)
                    ...
                    //2.修改选择规则集／规则后进入详情页：
                    // 再次返回当前页面时，初始化 router 参数
                    if(this.details.detailName == 'group'){
                    	this.$route.params.pageName = 'ruleSetConfig';
                    }
            ...
            getAssociatedData(){
            	if (this.details.pagePointing == 'product') {
                	this.sendRequest(this,'/product/productGoodsCatagory?select_rows={"associated":'+this.details.rawData.id+'}','rulesetsAllData');
                } else {
                	this.details.rawData.values.length = 0;
                	this.sendRequest(this,'/ruleset/rulesetGoodsCatagory?select_rows={"associated":'+this.details.rawData.id+'}','responseMessage','associatedRule');
                }
            },
            ...
            saveChangesFn(isConfirm) {// 提交当前修改
                if(isConfirm){
                    ...
                    let key = (this.details.detailName == 'product'?'status':'status')
                	this.details.variableData.values.map((item) => {
                		...
                		updateData.associatedData.push({
                            "id": item.id,
                            "state": item[key] || 0 // 状态不存在就设为 默认状态
                        });
                	})
                    ...
                    if (this.details.detailName == 'product') {
                    	if(updateData.associatedData.length == 1){
                    		this.cancelChanges();
                    		return alert('至少有 "两条规则集"， 或者一个都没有');
                    	}
                    	if(this.details.variableData.state == '上架中'){
                    	...
                    	updateData["group_id"] = this.details.variableData.id;
            ...
            deleteProduct() {// 删除产品或规则集
            	let str = this.details.detailName == 'product'? '产品':'规则集';
            ...
            deleteRule(id) { // 删除规则
            	this.$global.$splice(this.details.variableData.values,id);
                ...
                switch (msg.action) {
                    case 'remove':
                        if (msg.val.status == null || msg.val.status == '0') {
                            ...
                            this.$global.$splice(this.details.variableData.values,msg.val.id);
//                          
//                          for (var i = 0; i < this.details.variableData.values.length; i++) {
...
//                          }
            ...
            changeExcutingState(isConfirm) {// “执行中”规则集切换
                if (isConfirm) {
                    // 将其他正在执行的规则集切换为"未执行"；
					this.details.variableData.values.map((item) => {
						// 找到所有规则集中类型 == 当前点击的规则集类型，并且状态为“1” 的规则集。改变它的状态为“0”
						if(this.changeStatusParams.val.isCloud == item.isCloud && item.status == 1){   
                            item.status = '0';
                    		item.action_state1 = 2;
                    		item.action_state2 = 1;
                        }
					})
            ...
            setstate(data) {// 给包含的所有规则集各添加三组状态。( 执行中 = 2、 并行测试 = 1、 未执行 = 0)
            	data.correlation_id = [];
